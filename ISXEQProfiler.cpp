...
#include "ISXEQProfiler.h"

// The mandatory pre-setup function.  Our name is "ISXEQProfiler", and the class is ISXEQProfiler.
...
// our instanced class.
ISXPreSetup("ISXEQProfiler", ISXEQProfiler);
...
unsigned int ISXEQProfilerXML = 0;
...
bool ISXEQProfiler::Initialize(ISInterface *p_ISInterface)
{
	...
	WriteChatf("ISXEQProfiler Loaded");
	return true;
}

// shutdown sequence
void ISXEQProfiler::Shutdown()
{
	// save settings, if you changed them and want to save them now.  You should normally save
	// changes immediately.
	//pISInterface->ExportSet(ISXEQProfilerXML,XMLFileName);

	pISInterface->UnloadSet(ISXEQProfilerXML);
...
void ISXEQProfiler::ConnectServices()
{
...
void ISXEQProfiler::RegisterCommands()
{
	// add any commands
...
void ISXEQProfiler::RegisterAliases()
{
	// add any aliases
}

void ISXEQProfiler::RegisterDataTypes()
{
	// add any datatypes
	// pMyType = new MyType;
...
void ISXEQProfiler::UnRegisterCommands()
{
	// remove commands
	//    pISInterface->RemoveCommand("MyCommand");
}
void ISXEQProfiler::UnRegisterAliases()
{
	// remove aliases
}
void ISXEQProfiler::UnRegisterDataTypes()
{
	// remove data types
	//if (pMyType)
	//{
...
void ISXEQProfiler::UnRegisterTopLevelObjects()
{
	// remove Top-Level Objects
	//    pISInterface->RemoveTopLevelObject("MapSpawn");
}
void ISXEQProfiler::UnRegisterServices()
{
	// shutdown our own services
	//    if (hISXEQProfilerService)
	//        pISInterface->ShutdownService(this,hISXEQProfilerService);
}

void ISXEQProfiler::LoadSettings()
{
	bool bChanged = false;

	// open the settings file, and store the ID number in ISXEQProfilerXML
	ISXEQProfilerXML = pISInterface->OpenSettings(XMLFileName);
	...
	Any set can be referred to by its ID.  The base set is stored in ISXEQProfilerXML.  Note that the settings
	...
	*/
	...
	if (unsigned int GeneralGUID = pISInterface->FindSet(ISXEQProfilerXML, "General"))
	{
